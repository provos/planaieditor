...
export async function writeFile(path: string, content: string): Promise<{ message: string; path: string }> {
    ...
    try {
        ...
        if (!responseData || typeof responseData.message !== 'string' || typeof responseData.path !== 'string') {
...
export async function listDirectory(path: string): Promise<ListDirectoryResponse> {
    if (typeof path !== 'string') { // Allow empty string for root potentially, backend handles logic
    ...
    try {
        ...
        if (!response.ok) {
            const errorMessage =
                responseData.message || `Failed to list directory: ${response.status} ${response.statusText}`;
            console.error(`listDirectory error for path "${requestPath}": ${errorMessage}`);
            throw new Error(errorMessage);
        }

        // Validate the expected response structure
        if (!responseData || typeof responseData.path !== 'string' || !Array.isArray(responseData.items)) {
        ...
        // If it's not an error we already created, log it as a network error
        if (!(error instanceof Error && (error.message.startsWith('Failed to list directory:') || error.message.startsWith('listDirectory:')))) {
