...
export const llmConfigsFromCode: Writable<LLMConfigFromCode[]> = persisted<LLMConfigFromCode[]>(
	'llmConfigsFromCode',
	[]
);
...
export function updateLLMConfig(updatedConfig: LLMConfig) {
	llmConfigs.update((configs) => {
		...
		if (
			existingIndex !== -1 &&
			configs.some((c, index) => c.name === updatedConfig.name && index !== existingIndex)
		) {
			throw new Error(`Configuration name \'${updatedConfig.name}\' already exists.`);
		}

		return configs.map((config) => (config.id === updatedConfig.id ? updatedConfig : config));
	});
}
